  ...
  if (id) {
    ...
    try {
      ...
      res.skus?.forEach((item) => {
        if (isDetail.value) {
          console.log("122")
          item.price = Number(floatToFixed2(item.price))
          item.marketPrice = Number(floatToFixed2(item.marketPrice))
          item.costPrice = Number(floatToFixed2(item.costPrice))
          item.firstBrokeragePrice = Number(floatToFixed2(item.firstBrokeragePrice))
          item.secondBrokeragePrice = Number(floatToFixed2(item.secondBrokeragePrice))
        } else {
          // 回显价格分转元
