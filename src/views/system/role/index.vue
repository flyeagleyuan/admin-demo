    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column align="center" label="角色类型" prop="type">
        <template #default="scope">
          <!-- @vue-expect-error -->
          {{
            getIntDictOptions(DICT_TYPE.ROLE_STATUS).find((item) => item.value === scope.row.type)
              .label
          }}
        </template>
      </el-table-column>
...
const handleSwitchChange = (status, row) => {
  console.log(status, row, 'xxx')
  RoleApi.updateRoleStatus({ id: row.id, status }).then((res) => {
    if (res) {
